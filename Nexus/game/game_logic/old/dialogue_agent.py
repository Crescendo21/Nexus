...
class DialogueAgent:
    ...
    def send(self, message_content: str = None) -> str:  # Ajoutez l'argument ici
        """
        Applies the chatmodel to the message history
        and returns the message string
        """
        if self.is_user_controlled:
            if message_content is None:  # Si aucun message n'est fourni, demandez-le
                message_content = input(f"{self.name}, que voulez-vous faire? ")
            return message_content  # Renvoyez le message fourni
        else:
            message = self.model(
                [
                    self.system_message,
